...
4. 🟡 Add recent activity and social proof to Standings. — Standings now shows a gap-to-pass line in the viewer card and a Recent activity card served by `app/api/leaderboard/recent-activity/route.ts` with the latest season trades and an "active today" badge.
