...
2. ✅ Add share buttons to Standings and post-trade/status moments. — Standings shares the rank OG card; the wallet recent-trades list now has a per-row "Share trade" button (`components/pages/wallet/index.tsx` `TradeRow`) that embeds the dynamic trade OG card and deep-links the composer back to the relevant token. The Commodus card swaps to "Share the kill" with the boss card embed once the viewer is ahead.
3. ✅ Add dynamic OG score, rank, trade, and boss cards. — Rank card at `app/api/og/rank/[fid]/route.tsx`, trade card at `app/api/og/trade/[executionId]/route.tsx` (data via `lib/sharing/trade-card.ts`), boss-beaten card at `app/api/og/boss/[fid]/route.tsx`. URL helpers added in `lib/commodus/deep-links.ts` (`tradeCardImageUrl`, `bossCardImageUrl`). Standings-snapshot card deferred — not required for the launch loop.
4. 🟡 Add recent activity and social proof to Standings. — Standings now shows a "X pts to pass @next" line in the viewer card and a Recent activity card (`getRecentActivity()` in `lib/leaderboard/service.ts`, served by `app/api/leaderboard/recent-activity/route.ts`) with the latest scored trades and an "active today" badge. "Points to pass Commodus" was already in place. Scored-this-month signal still pending.
